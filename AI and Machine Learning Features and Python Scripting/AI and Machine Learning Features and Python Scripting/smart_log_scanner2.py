...
class SmartLogScanner:
    ...
    def load_log_file(self, log_file_path):
        """Load and parse log file with adaptive size handling for large files"""
        print(f"[*] Loading log file: {log_file_path}")

        try:
            file_size = os.path.getsize(log_file_path)
            
            # For very large log files (>500MB), sample instead of reading entire file
            if file_size > 500 * 1024 * 1024:  # > 500MB
                print(f"[*] Large log file detected ({file_size / (1024*1024):.1f}MB), sampling...")
                logs = []
                with open(log_file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    # Read first 10k lines
                    for i, line in enumerate(f):
                        if i < 10000 or (i % 100 == 0):  # Every 100th line after 10k
                            clean_line = line.strip()
                            if clean_line:
                                logs.append(clean_line)
                        if i > 1000000:  # Safety limit: 1M lines read
                            break
            elif file_size > 100 * 1024 * 1024:  # 100-500MB: read first 50% only
                print(f"[*] Medium log file detected ({file_size / (1024*1024):.1f}MB), partial read...")
                logs = []
                read_limit = 50 * 1024 * 1024  # Read first 50MB
                with open(log_file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    bytes_read = 0
                    for line in f:
                ...
                # Normal files: read entire content
                with open(log_file_path, 'r', encoding='utf-8', errors='ignore') as f:
