...
class TimelineBuilder:
    ...
    def parse_file_system_timeline(self, timeline_file):
        """Parse file system MAC timeline with adaptive size handling"""
        print(f"[*] Parsing file system timeline: {timeline_file}")

        try:
            file_size = os.path.getsize(timeline_file)
            
            # For large files, limit processing
            if file_size > 500 * 1024 * 1024:  # > 500MB
                print(f"[*] Large timeline file ({file_size / (1024*1024):.1f}MB), sampling...")
                line_count = 0
                with open(timeline_file, 'r', encoding='utf-8', errors='ignore') as f:
                    reader = csv.DictReader(f)
                    for idx, row in enumerate(reader):
                        if idx < 10000 or (idx % 100 == 0):  # Every 100th line after 10k
                            try:
                                event = {
