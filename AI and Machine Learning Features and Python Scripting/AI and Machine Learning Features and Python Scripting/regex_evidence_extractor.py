...
class RegexEvidenceExtractor:
    ...
    def extract_from_file(self, file_path, pattern_names=None):
        """Extract evidence from a file with adaptive size handling"""
        print(f"[*] Scanning file: {os.path.basename(file_path)}")

        try:
            file_size = os.path.getsize(file_path)
            
            # For very large files (>500MB), sample instead of reading entire file
            if file_size > 500 * 1024 * 1024:  # > 500MB
                print(f"[*] Large file detected ({file_size / (1024*1024):.1f}MB), sampling...")
                content = ""
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    # Read first 50MB
                    content = f.read(50 * 1024 * 1024)
                    if file_size > 100 * 1024 * 1024:
                        # Also read middle section
                        f.seek(file_size // 2)
                        content += "\n[...MID-FILE...]\n"
                        content += f.read(50 * 1024 * 1024)
            elif file_size > 100 * 1024 * 1024:  # 100-500MB
                print(f"[*] Medium file detected ({file_size / (1024*1024):.1f}MB), partial read...")
                content = ""
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read(50 * 1024 * 1024)  # First 50MB
            else:
